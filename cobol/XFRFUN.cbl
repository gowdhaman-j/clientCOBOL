 ...
 *******      EXEC SQL INCLUDE SQLDA END-EXEC.
      ...
      *
      *    Is the Account datastore held on DB2 if so, update the
      *    account. If the datastore is held on anything else then this
      *    is an error.
      *
           EVALUATE ACCOUNT-FLAG
              WHEN '2'
      ...
      *
                 ...
                 STRING 'A010 - Unknown datastore type '
                      DELIMITED BY SIZE,
                      DATA-STORE-TYPE DELIMITED BY SIZE,
                      '.' DELIMITED BY SIZE,
                      ' For ACCOUNT data.'
                      DELIMITED BY SIZE,
                 ...
                 DISPLAY 'ERROR in XFRFUN. Unknown datastore type '
                 ACCOUNT-FLAG
      ...
      *
      *    If the PROCTRAN_FLAG is set to '2' then we need to
      *    use the DB2 DATASTORE for PROCTRAN, if it is set
      *    to anything else, then we need to flag this as an
      *    error
      *
